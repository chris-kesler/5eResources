...
$(function populatePageTitle() {
	...
	if (page_name !== "entry-page.html") {
		document.getElementById("page-title-path").innerHTML += "<br>" + titleCase(page_name.replaceAll('-', ' ').replaceAll('.html', ''));
	} else if (page_name === "entry-page.html" && page_args != undefined) {
		document.getElementById("page-title-path").innerHTML += "<br>" + titleCase(page_args.replaceAll('-', ' ').replaceAll('.html', ''));
	} else {
		document.getElementById("page-title-path").innerHTML += "Hmm, can't seem to read the filename for some reason";
